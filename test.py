...
class AdvancedTemplateMatcher:
    ...
    def _detect_edges(self, image: np.ndarray) -> np.ndarray:
        """Detect edges in the template for better matching"""
            ...
            for card_name, template_dict in self.templates.items():
                for source_type, source_img in enhanced_source.items():
                    for template_type, template_img in template_dict.items():
                        ...
                        # Find the best confidence for this template
            ...
            if best_confidence > confidence_threshold:
                ...
                if rank and suit:
                    ...
                    if combined_match == best_match:
                        self.logger.log(
                            f"Validated match: {best_match} with confidence {best_confidence:.3f} using {best_method}")
                        return best_match
                    else:
    ...
    def match_rank_and_suit_separately(self, card_image: np.ndarray, confidence_threshold: float = 0.7) -> Tuple[
        ...
        try:
            ...
            if best_rank_confidence > confidence_threshold and best_suit_confidence > confidence_threshold:
                self.logger.log(
                    f"Matched rank: {best_rank} ({best_rank_confidence:.3f}), suit: {best_suit} ({best_suit_confidence:.3f})")
                return best_rank, best_suit
            else:
                self.logger.log(
                    f"Separate matching failed. Rank: {best_rank} ({best_rank_confidence:.3f}), Suit: {best_suit} ({best_suit_confidence:.3f})")
                return None, None
    ...
    def create_templates_from_screenshot(self, screenshot_path: str, output_dir: str,
